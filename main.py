...
if __name__ == '__main__':
  ...
  parser.add_argument('-c', '--count', type=int, help='number of points in the distribution', 
                      default=50)
  parser.add_argument('-d', '--dimension', type=int, help='dimensional size of the space, '
                      'number of axes', default=400)
  parser.add_argument('-p', '--projected', type=int, help='projected dimensional size of the '
                      'space')
  parser.add_argument('-m', '--mean', type=float, help='target mean of the distribution', 
                      default=0.0)
  parser.add_argument('-s', '--standard_deviation', type=float, help='target standard deviation '
                      'of the distribution', default=1.0)
  ...
  if (args.projected and args.projected >= args.dimension):
    print('Error: The projected dimension must be smaller than the value specified by '
         '--dimension')
    sys.exit(1)
  if args.mean < 0.0:
  ...
  details = output_dir + '/c' + str(count) + '_d' + str(dimension) + '_m' + str(mean) + '_s' \
  ...
  with io.open(tfn, 'w', encoding='utf-8') as f:
    f.write(unicode(json.dumps(cluster, cls=highdim.GaussianEncoder, sort_keys=True, indent=4, 
                              ensure_ascii=False)))
  if args.verbose:
    print json.dumps(cluster, cls=highdim.GaussianEncoder, sort_keys=True, indent=4, 
                    ensure_ascii=False)
  ...
  highdim.GaussianStats.plot_distribution(o_dists, 'Distances From Origin', 'Distance', 'PDF', 
                                          legend, ifn)
  if args.verbose:
  ...
  highdim.GaussianStats.plot_distribution(list_dists, 'Distances Between Pairs', 'Distance', 'PDF', 
                                          legend, ifn)
  if args.verbose:
  ...
  list_angles, matrix_angles = cluster.cluster_distances()
  ...
  highdim.GaussianStats.plot_distribution(list_angles, 'Angles Between Pairs', 'Angle (degrees)', 'PDF', 
                                          legend, ifn)
  if args.verbose:
  ...
  details = output_dir + '/c' + str(count) + '_d' + str(projected) + '_m' + str(mean) + '_s' \
          + str(standard_deviation)
  ...
  with io.open(tfn, 'w', encoding='utf-8') as f:
    f.write(unicode(json.dumps(projected_cluster, sort_keys=True, indent=4, ensure_ascii=False)))
  if args.verbose:
    print json.dumps(projected_cluster, sort_keys=True, indent=4, ensure_ascii=False)
  ...
  list_angles, matrix_angles = projected_cluster.cluster_distances()
  ...
  list_dists, matrix_dists = highdim.GaussianStats.generate_scaled_distances(original_list_dists, projected)
  tfn = details + '_scaled_cluster_dists.json'
  ifn = details + '_scaled_cluster_dists.png'
  with io.open(tfn, 'w', encoding='utf-8') as f:
    f.write(unicode(json.dumps(list_dists, sort_keys=True, indent=4, ensure_ascii=False)))
  highdim.GaussianStats.plot_distribution(list_dists, 'Distances Between Pairs (Scaled)', 'Distance', 'PDF', 
                                          legend, ifn)
  if args.verbose:
    print json.dumps(list_dists, sort_keys=True, indent=4, ensure_ascii=False)

  # Calculate percentage error comparsing the scaled and projected characteristics
  details = output_dir + '/c' + str(count) + '_d' + str(dimension) + '_p' + str(projected) + '_m' + str(mean) \
          + '_s' + str(standard_deviation)
  ...
  err[0]['Average']['Original Dimension Scaled'] = \
    str(highdim.GaussianStats.pct_err_distance_average(original_list_dists, dimension, projected))
  err[0]['Average']['Projected Dimension'] = \
    str(highdim.GaussianStats.pct_err_distance_average(projected_list_dists, dimension, projected))
  err[0]['Max']['Original Dimension Scaled'] = \
    str(highdim.GaussianStats.pct_err_distance_max(original_list_dists, dimension, projected))
  err[0]['Max']['Projected Dimension'] = \
